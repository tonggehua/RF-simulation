...
if __name__ == '__main__':
    ...
    seq, bin_centers = write_2D_MSI(N=N,FOV=FOV,enc='xyz',TR=TR,TE=TE,thk=thk,turbo_factor=tf,slice_locations=sl_locs,
                       N_bins=N_bins,bin_width=bin_width,t_ex=dur_factor*2.5e-3, t_ref=dur_factor*2e-3, randomize_bin=randbin,
                                    use_sigpy_180=True)
    print("Done")
    # Check sequence
    ...
    # Save
    #seq.write(f'2D_MSI_sigpy_031422_TR{TR}_Ns{len(sl_locs)}_random_{randbin}_slicerev{slicerev}_Nbins{N_bins}_bw{N_bins*bin_width}_disp{displacement}_rfdur{dur_factor}x_tf{tf}.seq')
    #savemat('2D_MSI_031422_bin_centers_for_randbin.mat',{'bin_centers': bin_centers})
