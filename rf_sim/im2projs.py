...
def image_to_projections(image, n_proj, rf, grad, dt):
    ...
    for ind, theta in enumerate(thetas_rf):
        ...
        P_rf.append(rf_sim_proj(all_spins, rf, grad, dt, theta, 100))
        print('Theta #{:d} simulated'.format(ind+1))
        print('time taken: ', str(time.time()-last_time))
  ...
  #  P_rf = np.zeros((n_points, n_proj), dtype=complex)
   # for u in range(n_proj):
    #    kline = np.concatenate((np.flip(P_rf_radial[1:,u+n_proj],axis=0),P_rf_radial[0:-1,u]))
     #   P_rf[:,u] = np.fft.fftshift(np.fft.ifft(kline))
...
def sample_rf_proj_correct(signal, dt, n_samples):
...
if __name__ == '__main__':
    ...
    Pim, Prf = image_to_projections(dog, n_proj, rf, grad, dt=dt)

    print('Time taken: ', str(time.time()-beginning), ' seconds')
    savemat('./rf_sim/projs_dog.mat',{'PROJ_IM': Pim, 'PROJ_RF': Prf})
